...
export class SearchExecutor {
  ...
  async executeSearch(
    ...
    // If no specific parameters were set, decide based on query content
    if (!useCondParam && !useTermParam) {
      // Check if this looks like a medical/mutation query that should use query.cond
      const medicalTerms = ['kras', 'egfr', 'alk', 'ros1', 'braf', 'mutation', 'cancer', 'tumor', 'carcinoma'];
      const hasMedicalTerms = medicalTerms.some(term => queryLower.includes(term));
      
      if (hasMedicalTerms) {
        // Use query.cond for medical/mutation queries
        params.append('query.cond', query);
      } else {
        // Use query.term for general searches
